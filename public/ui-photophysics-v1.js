/* CuHalide Atlas structured photophysics browser layer v1.4.0. */
(() => {
  'use strict';
  ...
  function stageLabel(ph){if(ph?.public_state==='two_pass_verified'||ph?.two_pass_verified===true)return 'Curated';if(ph?.public_state==='verified_no_reported_data')return 'Reviewed · no reported data';return 'Data unavailable'}
  ...
  function injectHero(){
    ...
    if(tags&&!tags.querySelector('.photo-contract-tag'))tags.insertAdjacentHTML('beforeend','<span class="photo-contract-tag">Sample-resolved photophysics</span>');
  }
  ...
  function injectHomePanel(){
    ...
    dashboard.insertAdjacentHTML('afterbegin',`<article class="panel photo-home-panel">
      ...
      <div class="photo-home-metrics" id="photoHomeMetrics"><div class="photo-metric"><strong>—</strong><span>Samples</span></div><div class="photo-metric"><strong>—</strong><span>Measurements</span></div><div class="photo-metric"><strong>—</strong><span>Values</span></div></div>
    </article>`);
  }
  ...
  function injectView(){
    ...
    target.insertAdjacentHTML('beforebegin',`<section class="view photo-view" data-view="photophysics">
      <div class="shell page-head"><p class="eyebrow">Sample-resolved measurements</p><h1>Photophysics</h1><p>Explore photophysical measurements with sample form and experimental context preserved. Crystal, powder, composite, film and device results remain separate rather than being silently merged.</p></div>
      <div class="shell photo-status-grid" id="photoStatusGrid" aria-live="polite"><div class="panel loading">Loading photophysics…</div></div>
      <div class="shell photo-feature-grid">
        <article class="panel"><p class="eyebrow">Sample context</p><h2>Compare like with like</h2><p class="fine">Measurements are attached to the reported sample state, so intrinsic crystal data are not conflated with processed materials or devices.</p><div class="photo-grain-chain"><span>Article</span><i>→</i><span>Sample</span><i>→</i><span>Measurement</span><i>→</i><span>Value</span></div></article>
        <article class="panel"><p class="eyebrow">Source review</p><h2>Conflicting values remain explicit</h2><p class="fine">When primary sources disagree, the discrepancy is shown rather than silently averaged or replaced. Values suitable for quantitative comparison are tracked separately.</p></article>
      </div>
      <div class="shell photo-example-panel panel"><div><p class="eyebrow">Examples</p><h2>Inspect structured measurements directly</h2><p class="fine">Open an article to see its sample → measurement → value hierarchy and any source discrepancy that affects interpretation.</p></div><div class="photo-example-actions"><button class="btn primary" type="button" data-article="381">Open Record 381</button><button class="btn secondary" type="button" data-article="46">Open PLQY example · Record 46</button><a class="btn secondary" href="#articles">Search literature</a></div></div>
    </section>`);
  }
  ...
  function mechanismLabels(sample){const x=[];(sample.measurements||[]).forEach(m=>(m.mechanisms||[]).forEach(z=>{const l=z.label||z.mechanism_code;if(l&&!x.includes(l))x.push(l)}));return x}
  function bandFacts(sample){const facts=[];(sample.measurements||[]).forEach(m=>(m.bands||[]).forEach(b=>{if(b.peak_nm&&['emission','radioluminescence','cpl'].includes(String(b.domain||''))){const k=`${b.domain}:${b.peak_nm}`;if(!facts.some(x=>x.key===k))facts.push({key:k,label:b.domain==='emission'?'Emission peak':b.domain==='radioluminescence'?'RL peak':'CPL peak',value:`${num(b.peak_nm)} nm`})}}));return facts}
  function sampleCard(s,index){
    ...
    const facts=[...bands.map(x=>`<div class="photo-fact"><span>${esc(x.label)}</span><strong>${esc(x.value)}</strong></div>`),...vals.map(v=>`<div class="photo-fact"><span>${esc(labelKey(v.property_key))}</span><strong>${esc(fmtValue(v))}</strong></div>`)].slice(0,10).join('');
    const sampleForm=s.sample_form?String(s.sample_form).replaceAll('_',' '):'';
    const measurementTypes=[...new Set((s.measurements||[]).map(m=>String(m.measurement_type||'').replaceAll('_',' ')).filter(Boolean))];
    return `<article class="photo-sample"><div class="photo-sample-head"><div><p class="eyebrow">Sample ${index+1}</p><h3>${esc(s.sample_label||s.reported_compound_label||'Curated sample')}</h3><p class="fine">${esc(sampleForm)}${s.structure_id?` · ${esc(s.structure_id)}`:''}</p></div><span class="photo-count">${esc((s.measurements||[]).length)} measurements</span></div>${facts?`<div class="photo-facts">${facts}</div>`:''}${mechs.length?`<p class="photo-mechanisms"><strong>Mechanism:</strong> ${mechs.map(esc).join(' · ')}</p>`:''}${measurementTypes.length?`<details><summary>Measurement types and conditions</summary><p class="fine">${measurementTypes.map(esc).join(' · ')}</p>${(s.measurements||[]).filter(m=>m.conditions).slice(0,4).map(m=>`<p class="fine"><strong>${esc(String(m.measurement_type||'measurement').replaceAll('_',' '))}:</strong> ${esc(m.conditions)}</p>`).join('')}</details>`:''}</article>`;
  }
  function renderPhotophysics(ph){
    const counts=ph.counts||{},samples=Array.isArray(ph.samples)?ph.samples:[];
    if(ph.public_state==='verified_no_reported_data')return `<section class="photo-modal-section"><div class="photo-modal-title"><div><p class="eyebrow">Photophysics</p><h2>No reported measurement</h2></div><span class="photo-stage passa">Reviewed · no reported data</span></div><p class="fine">No reportable photophysics measurement was found for the reviewed article.</p></section>`;
    if(!publishableArticleState(ph)||ph.two_pass_verified!==true)return `<section class="photo-modal-section"><div class="photo-modal-title"><div><p class="eyebrow">Photophysics</p><h2>Data not available</h2></div></div><p class="fine">Structured measurements are not available for this article.</p></section>`;
    const conflicts=Array.isArray(ph.conflicts)?ph.conflicts:[];
    return `<section class="photo-modal-section"><div class="photo-modal-title"><div><p class="eyebrow">Photophysics</p><h2>Sample-resolved measurements</h2></div><span class="photo-stage ${stageClass(ph)}">${esc(stageLabel(ph))}</span></div><div class="photo-inline-stats"><span><strong>${esc(counts.samples??samples.length)}</strong> samples</span><span><strong>${esc(counts.measurements??0)}</strong> measurements</span><span><strong>${esc(counts.values??0)}</strong> values</span></div><div class="photo-samples">${samples.slice(0,8).map(sampleCard).join('')}</div>${samples.length>8?`<p class="fine">${esc(samples.length-8)} additional sample states are available through search.</p>`:''}${conflicts.length?`<div class="photo-conflict"><strong>Source discrepancy</strong>${conflicts.slice(0,4).map(c=>`<p>${esc(labelKey(c.property_key))}: ${esc(c.warning||'Conflicting source values are retained explicitly.')}</p>`).join('')}</div>`:''}</section>`;
  }
  ...
  async function enhanceArticleModal(){
    ...
    const loading=document.createElement('section');loading.className='block full photo-modal-loading';loading.innerHTML='<h3>Photophysics</h3><p class="fine">Loading sample-resolved measurements…</p>';grid.appendChild(loading);
    try{
      ...
      loading.outerHTML=x.photophysics&&x.photophysics.ok!==false?renderPhotophysics(x.photophysics):'<section class="photo-modal-section"><div class="photo-modal-title"><div><p class="eyebrow">Photophysics</p><h2>No structured measurements</h2></div></div><p class="fine">No structured photophysics record is available for this article.</p></section>';
    }catch(e){if(loading.isConnected)loading.innerHTML=`<h3>Photophysics</h3><p class="fine">Temporarily unavailable: ${esc(e.message)}</p>`}
  }
  ...
  async function init(){
    ...
    try{renderHealth(await getHealth())}catch(e){const n=$('photoStatusGrid');if(n)n.innerHTML='<div class="error">Photophysics is temporarily unavailable.</div>'}
  }
