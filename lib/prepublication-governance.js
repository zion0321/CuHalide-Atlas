...
function governRecordJsonLd(body){
  ...
  const out=body.replace(re,(full,raw)=>{
    ...
    return governed?`<script type="application/ld+json">${JSON.stringify(value)}</script>`:full;
  });
  if(!governed)throw new Error('prepublication governance: Atlas record JSON-LD scope not found');
  return out;
}
...
export function applyRootPrepublicationGovernance(input){
  ...
  body=all(body,'CuHalide Atlas. Continuously curated Cu(I) halide knowledge portal. Curated through 19 August 2026. https://cuhalide-atlas-v3.vercel.app/','CuHalide Atlas. Prepublication review resource. Current Curated rev.7, curated through 19 August 2026. https://cuhalide-atlas-v3.vercel.app/. Include the access date when referenced.');
  ...
  for(const token of [STATE_META,ROOT_DESCRIPTION,REVIEW_OG_ALT,'"creativeWorkStatus":"Prepublication review"','Prepublication review interface','Prepublication attribution','Review-access knowledge layer','Review access is query-and-view.'])if(!body.includes(token))throw new Error(`prepublication governance: required root token missing: ${token}`);
  for(const stale of ['structure-resolved public knowledge portal','<summary>Public scientific interface</summary>','<p class="eyebrow">Recommended citation</p>','<h2>Public knowledge layer</h2>','Public access is query-and-view.'])if(body.includes(stale))throw new Error(`prepublication governance: ambiguous formal-release copy remains: ${stale}`);
  return body;
}
...
export function applyRecordPrepublicationGovernance(input){
  ...
  body=governRecordJsonLd(body);
  for(const token of [STATE_META,REVIEW_OG_ALT,'latest curated review-access record','Review pages expose only field-whitelisted information','"creativeWorkStatus":"Prepublication review"'])if(!body.includes(token))throw new Error(`prepublication governance: required record token missing: ${token}`);
  if(body.includes('latest curated public record')||body.includes('Public pages expose only field-whitelisted information'))throw new Error('prepublication governance: stale record access wording remains');
  return body;
}
