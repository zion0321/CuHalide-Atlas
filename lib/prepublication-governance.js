...
export function applyRootPrepublicationGovernance(input){
  ...
  body=all(body,'CuHalide Atlas. Continuously curated Cu(I) halide knowledge portal. Curated through 19 August 2026. https://cuhalide-atlas-v3.vercel.app/','CuHalide Atlas. Prepublication review resource. Current Curated rev.10, curated through 14 September 2026. https://cuhalide-atlas-v3.vercel.app/. Include the access date when referenced.');
  body=all(body,'CuHalide Atlas. Prepublication review resource. Current Curated rev.7, curated through 14 September 2026. https://cuhalide-atlas-v3.vercel.app/. Include the access date when referenced.','CuHalide Atlas. Prepublication review resource. Current Curated rev.10, curated through 14 September 2026. https://cuhalide-atlas-v3.vercel.app/. Include the access date when referenced.');
  ...
  for(const token of [STATE_META,ROOT_DESCRIPTION,REVIEW_OG_ALT,'"creativeWorkStatus":"Prepublication review"','Prepublication review interface','Prepublication attribution','Review-access knowledge layer','Review access is query-and-view.','Current Curated rev.10'])if(!body.includes(token))throw new Error(`prepublication governance: required root token missing: ${token}`);
  for(const stale of ['structure-resolved public knowledge portal','<summary>Public scientific interface</summary>','<p class="eyebrow">Recommended citation</p>','<h2>Public knowledge layer</h2>','Public access is query-and-view.','Current Curated rev.7, curated through 14 September 2026'])if(body.includes(stale))throw new Error(`prepublication governance: ambiguous formal-release copy remains: ${stale}`);
  return body;
}
