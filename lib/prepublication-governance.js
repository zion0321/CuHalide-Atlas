...
export function applyRecordPrepublicationGovernance(input){
  ...
  for(const token of [STATE_META,REVIEW_OG_ALT,'latest curated review-access record','Review pages expose only field-whitelisted information'])if(!body.includes(token))throw new Error(`prepublication governance: required record token missing: ${token}`);
