...
test('public Rev.8 copy contains no active Rev.7 or Rev.6 residues',async({request})=>{
  for(const path of ['/api/site','/']){
    ...
    expect(html).toContain('Rev.8 incorporates primary-source-reverified structure corrections');
    expect(html).toContain('cc.live_revision||8');
    expect(html).not.toContain('Current Curated rev.6');
    expect(html).not.toContain('Current Curated rev.7');
    expect(html).not.toContain('Curated through 18 August 2026');
    expect(html).not.toContain('cc.live_revision||6');
    expect(html).not.toContain('cc.live_revision||7');
    ...
    expect(html).not.toContain('Rev.7 completes a full structure-truth re-audit across the 946-row Current Curated snapshot');
  }
  ...
  if(CANDIDATE_HOST){
    ...
    expect(health.headers()['x-cuhalide-current-curated-revision']).toBe('8');
    const h=await health.json();
    expect(h.site_probe_mode).toBe('frontend v50 active; backend Current Curated rev.8 deterministic contract; Structured Photophysics 1.3.3 staged publication; Organic Components 1.1.0 structure-grain fail-closed depiction contract');
