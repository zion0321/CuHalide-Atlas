...
const CURRENT_MIDDLEWARE='release-3.0.2-ui-v50.2-current-r8';
...
test('stale UI function suffixes terminate at the current assistant renderer',async({request},testInfo)=>{
  ...
  for(const path of ['/api/ui-site.js','/api/ui-assistant.js']){
    ...
    expect(html,path).toContain('Current Curated rev.8');
    ...
    expect(html,path).not.toContain('Current Curated rev.7');
  }
});
...
test('stale record function suffixes terminate at the evidence-grain wrapper',async({request},testInfo)=>{
  ...
  for(const path of ['/api/record.js','/api/record-current.js']){
    ...
    expect(header(r,'x-robots-tag'),path).toContain('noindex');
    const html=await r.text();
    expect(html,path).toContain('Current Curated rev.8 · primary-evidence reviewed through 19 Aug 2026');
    ...
    expect(html,path).not.toContain('Current Curated rev.6');
    expect(html,path).not.toContain('Current Curated rev.7 · primary-evidence reviewed');
...
test('legacy record suffix preserves current article provenance and promoted two-pass photophysics',async({request},testInfo)=>{
  ...
  const html=await r.text();
  expect(html).toContain('Current Curated rev.8 context · core article record inherited from immutable Frozen Release 3.0.2 baseline');
...
test('final wrapper function suffixes remain safe direct positive controls',async({request},testInfo)=>{
  ...
  expect(header(ui,'x-cuhalide-current-curated-revision')).toBe('8');
  const uiHtml=await ui.text();
  expect(uiHtml).toContain('Current Curated rev.8');
  ...
  expect(uiHtml).not.toContain('Current Curated rev.7');
  const record=await request.get(`${BASE}/api/record-evidence-current.js?kind=structure&id=CUH-006-S01`);
  expect(record.status()).toBe(200);
  expect(header(record,'x-cuhalide-current-curated-revision')).toBe('8');
