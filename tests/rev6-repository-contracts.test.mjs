...
test('v50 public runtime files expose one rev.6 contract',()=>{
  ...
  for(const token of ["CURRENT_REVISION='6'","2026-08-18","946","886","687","85","1329"])assert.ok(text.includes(token),`missing ${token}`);
...
test('record pages cannot regress to rev.3/site48 provenance',()=>{
  ...
  for(const token of ["SITE_VERSION='50'","CURRENT_REVISION='6'","CURRENT_DATE='2026-08-18'",'Current Curated rev.6','Retry-After','X-Robots-Tag'])assert.ok(record.includes(token),`record contract missing ${token}`);
...
test('health Motif sitemap and data gateways retry transient faults without hiding persistent failure',()=>{
  ...
  assert.match(meta,/RETRIES=3/);assert.match(meta,/frontend v50 active; backend rev\.6 deterministic contract/);assert.doesNotMatch(meta,/frontend v49 active/);
...
test('legacy public data route is compatibility-only but current',()=>{
  ...
  for(const token of ["PUBLIC_DATA_VERSION='2.13.0'","CURRENT_REVISION='6'",'prefer /api/public-data','RETRIES=3'])assert.ok(data.includes(token),`legacy data contract missing ${token}`);
  assert.ok(!data.includes("CURRENT_REVISION='3'"));
});

test('sitemap and QA denominators are exact',()=>{
  const sitemap=read('api/sitemap.js'),qa=read('tests/production-browser-v50.spec.js'),site=read('api/site.js'),siteQa=read('tests/site-quality-v50-1.spec.js');
