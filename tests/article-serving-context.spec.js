...
async function fetchArticle(request,path,id){const r=await request.get(`${BASE}${path}?action=article&id=${id}`);expect(r.status()).toBe(200);expect(r.headers()['x-cuhalide-current-curated-revision']).toBe('8');return{response:r,json:await r.json()}}

test('frozen-origin article keeps origin while declaring current rev.8 serving and promoted two-pass photophysics context',async({request})=>{
  ...
  expect(x.record_context).toEqual({serving_context:'current_curated',serving_revision:8,core_record_origin:'frozen_release',core_record_origin_release:'3.0.2',core_record_origin_revision:null,attached_photophysics_context:'current_curated',attached_photophysics_contract:'1.3.3',context_policy:'core_origin_preserved_current_overlays_explicit'});
  assertNoPrivateEvidence(x);
});

test('current-origin article uses the same rev.8 serving contract without inventing a frozen origin',async({request})=>{
  ...
  expect(x.item?.live_revision).toBe(8);
  expect(x.photophysics?.public_state).toBe('two_pass_verified');
  expect(x.record_context?.serving_context).toBe('current_curated');
  expect(x.record_context?.serving_revision).toBe(8);
  ...
  expect(x.record_context?.core_record_origin_revision).toBe(8);
  expect(x.record_context?.attached_photophysics_context).toBe('current_curated');
  expect(x.record_context?.attached_photophysics_contract).toBe('1.3.3');
  expect(x.record_context?.context_policy).toBe('core_origin_preserved_current_overlays_explicit');
  assertNoPrivateEvidence(x);
});

test('legacy /api/data is a compatibility alias of the canonical rev.8 article context',async({request})=>{
