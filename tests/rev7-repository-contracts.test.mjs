...
test('canonical public runtime exposes one rev.7 contract',()=>{
  ...
  for(const token of ["CURRENT_REVISION='7'","2026-08-19","946","886","684","87","1329","META_VERSION='50.5'","PUBLIC_DATA_VERSION='2.16.0'","PHOTOPHYSICS_CONTRACT='1.3.0'","prepublication-review"])assert.ok(text.includes(token),`missing ${token}`);
...
test('README is locked to the same rev.7 living scientific state',()=>{
  ...
  for(const token of ['Prepublication release status','Current Curated rev.7','2026-08-19','Article audit records | 383','Canonical verified articles | 369','Structure / phase rows | 946','Core-Included structure rows | 886','Resolved space-group rows | 710','Verified one-to-one SG rows | 684','Strict-polar rows | 87','RAG documents / embeddings | 1,329 / 1,329','628','318','Metadata gateway: **50.5**','Public Data: **2.16.0**','Structured Photophysics contract: **1.3.0**','Smart RAG: **9.19.0**','Research Assistant: **10.4.1**','Publication/governance state: **prepublication-review**','Pass A curated','two-pass verified'])assert.ok(readme.includes(token),`README missing current token ${token}`);
...
test('legacy data route remains compatibility-only and delegates to the current canonical contract',()=>{
  ...
  for(const token of ["PUBLIC_DATA_VERSION='2.16.0'","CURRENT_REVISION='7'","PHOTOPHYSICS_CONTRACT='1.3.0'","PUBLICATION_STATE='prepublication-review'",'cuhalide-atlas-public-data-v3'])assert.ok(publicData.includes(token),`canonical public-data missing ${token}`);
