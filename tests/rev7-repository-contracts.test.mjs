...
test('canonical public runtime exposes one active rev.8 contract',()=>{
  const files=['api/ui-site.js','api/ui-assistant-current.js','api/meta.js','api/data.js','api/public-data.js','api/sitemap.js','api/agent.js','api/motifs.js','api/record-evidence-current.js','middleware.js','vercel.json'];
  const text=files.map(read).join('\n');
  for(const token of ["CURRENT_REVISION='8'","2026-08-19","946","886","684","87","1329","META_VERSION='50.5'","PUBLIC_DATA_VERSION='2.16.0'","PHOTOPHYSICS_CONTRACT='1.3.3'","prepublication-review"])assert.ok(text.includes(token),`missing ${token}`);
...
test('README is the authoritative Rev.8 / Structured Photophysics 1.3.3 release document',()=>{
  const readme=read('README.md');
  for(const token of ['Prepublication release status','Current Curated rev.8','2026-08-19','Article audit records | 383','Canonical verified articles | 369','Structure / phase rows | 946','Core-Included structure rows | 886','Resolved space-group rows | 710','Verified one-to-one SG rows | 684','Strict-polar rows | 87','RAG documents / embeddings | 1,329 / 1,329','628','318','Metadata gateway: **50.5**','Public Data: **2.16.0**','Structured Photophysics contract: **1.3.3**','Smart RAG: **9.19.0**','Research Assistant: **10.4.1**','Publication/governance state: **prepublication-review**','940 publishable sample states','2,267 measurements','2,988 normalized values','281 quantitative-analysis-eligible values','477 mechanism claims','92 two-pass verified articles','237 Pass A curated articles','54 verified-no-data articles','CUH-285-S04','CUH-294-S03','Cu6I9 + Cu7I11','1D zigzag edge-sharing','Global Pass B is **not** complete'])assert.ok(readme.includes(token),`README missing locked Rev.8 token ${token}`);
  for(const stale of ['Current Curated rev.5 — living default','Curated through **2026-08-17**','Structure / phase rows | 938','Core-Included structure rows | 878','Public Data: **2.12.0**','Smart RAG: **9.17.0**','Research Assistant: **10.2.0**','Structured Photophysics contract: **1.3.1**','Structured Photophysics contract: **1.3.2**'])assert.ok(!readme.includes(stale),`README stale token ${stale}`);
});
...
test('production Photophysics readiness remains locked to 1.3.3',()=>{
  ...
  for(const stale of ['stable132','staged133','controlled 1.3.2 -> 1.3.3 migration states','publishable_measurements===2262','publishable_values===2985'])assert.ok(!workflow.includes(stale),`completed migration token must not remain: ${stale}`);
});

test('canonical routes terminate at Rev.8 evidence wrappers, while Rev.7 remains recovery-only',()=>{
  ...
  assert.match(middleware,/release-3\.0\.2-ui-v50\.2-current-r8/);
  assert.ok(middleware.includes("headers.set('x-cuhalide-current-curated-revision','8')"));
  assert.ok(middleware.includes("headers.set('x-cuhalide-publication-state','prepublication-review')"));
  assert.match(candidate,/release-3\.0\.2-ui-v50\.2-current-r8/);
  assert.match(candidate,/ui-assistant-current/);
  assert.match(candidate,/record-evidence-current/);
  assert.ok(fs.existsSync(url('supabase/functions/cuhalide-atlas-current-rag-r7-exact-internal/index.ts')),'locked rev7 recovery exact source must remain');
  assert.ok(fs.existsSync(url('supabase/functions/cuhalide-atlas-current-rag-r8-unified-internal/index.ts')),'active rev8 unified source missing');
});
...
test('canonical Rev.8 record wrapper locks visible Photophysics 1.3.3 while preserving evidence grain',()=>{
  const ui=read('api/ui-site.js'),assistant=read('api/ui-assistant-current.js'),record=read('api/record-evidence-current.js');
  for(const token of ['CUHALIDE_SITE_V50_CURRENT_CURATED_R8','Current Curated rev.8','19 Aug 2026','Smart RAG 9.19.0','cc.verified_space_group_rows||684','cc.strict_polar_rows||87','cc.strict_polar_articles||54'])assert.ok(ui.includes(token),`UI promotion missing ${token}`);
  ...
  assert.match(record,/x-cuhalide-current-curated-revision/);
  assert.match(record,/x-cuhalide-photophysics-contract/);
  assert.match(record,/lockCurrentRevision/);
  assert.match(record,/lockPhotophysicsContract/);
  assert.match(record,/Structured Photophysics 1\.3\.2/,'legacy lower-layer text must be explicitly normalized');
...
test('metadata health and manifest carry Rev.8 and Photophysics 1.3.3 without weakening governance',()=>{
  const meta=read('api/meta.js');
  for(const token of ["META_VERSION='50.5'","PUBLICATION_STATE='prepublication-review'","PUBLIC_DATA_VERSION='2.16.0'","PHOTOPHYSICS_VERSION='1.3.3'","CURRENT_REVISION='8'","curated_through:'2026-08-19'",'resolved_space_group_rows:710','verified_space_group_rows:684','verified_polar_rows:97','strict_polar_rows:87','strict_polar_articles:54','motif_resolved_rows:628','motif_unresolved_rows:318','photophysics_contract_version:PHOTOPHYSICS_VERSION',"smart_rag_version:'9.19.0'","research_assistant_version:'10.4.1'", "release_state:'prepublication'", "governance_state:PUBLICATION_STATE", "indexing:'disabled-prepublication'","structured_photophysics:true","photophysics_publication_policy='pass-a-curated-or-two-pass-verified'","public_projection:'pass-a-curated-or-two-pass-verified'","two_pass_identity_preserved:true","measurement_conflicts_fail_closed:true"])assert.ok(meta.includes(token),`metadata contract missing ${token}`);
  assert.match(meta,/frontend v50 active; backend Current Curated rev\.8 deterministic contract; Structured Photophysics 1\.3\.3 staged publication/);
  assert.doesNotMatch(meta,/backend Current Curated rev\.7 deterministic contract/);
});

test('assistant proxy exposes Rev.8 / 10.4 / 9.19 while preserving non-idempotent retry guard',()=>{
  const agent=read('api/agent.js');
  for(const token of ["ASSISTANT_VERSION='10.4.1'","EVIDENCE_VERSION='9.19.0'","CURRENT_REVISION='8'"])assert.ok(agent.includes(token));
  assert.match(agent,/attempts=isPost\?1:3/);
  assert.match(agent,/X-Robots-Tag/);
});

test('Motif Atlas reflects the adjudicated Rev.8 taxonomy without formula completion',()=>{
  const motifs=read('api/motifs.js'),config=JSON.parse(read('vercel.json'));
  for(const token of ["REV='8'","CONTENT_DATE='2026-08-19'",'motif_resolved_rows??628','motif_unresolved_rows??318',"version:'current-r8'",'Current Curated rev.8'])assert.ok(motifs.includes(token),`motif contract missing ${token}`);
...
test('prepublication sitemap preserves Rev.8 provenance without exposing record denominators as URLs',()=>{
  const sitemap=read('api/sitemap.js');
  for(const token of ["CONTENT_DATE='2026-08-19'","CURRENT_REVISION='8'",'prepublication-non-enumerating','X-CuHalide-Sitemap-URLs'])assert.ok(sitemap.includes(token),`sitemap contract missing ${token}`);
...
test('legacy data route remains compatibility-only and delegates to the current canonical contract',()=>{
  ...
  for(const token of ["PUBLIC_DATA_VERSION='2.16.0'","CURRENT_REVISION='8'","PHOTOPHYSICS_CONTRACT='1.3.3'","PUBLICATION_STATE='prepublication-review'",'cuhalide-atlas-public-data-v3'])assert.ok(publicData.includes(token),`canonical public-data missing ${token}`);
...
test('public privacy boundary remains query-and-view while verification stage is explicit',()=>{
  ...
  assert.match(readme,/Two-pass verified|two-pass verified/i);
