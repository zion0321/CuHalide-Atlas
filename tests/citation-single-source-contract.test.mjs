import test from 'node:test';
import assert from 'node:assert/strict';
import fs from 'node:fs';

const read=p=>fs.readFileSync(new URL(`../${p}`,import.meta.url),'utf8');

test('runtime citation metadata reads canonical repository CITATION.cff',()=>{
  const meta=read('api/meta-r9.js'),alias=read('api/meta.js'),config=JSON.parse(read('vercel.json')),cff=read('CITATION.cff');
  assert.match(meta,/import fs from 'node:fs'/);
  assert.match(meta,/CITATION_PATH=path\.join\(process\.cwd\(\),'CITATION\.cff'\)/);
  assert.match(meta,/fs\.readFileSync\(CITATION_PATH,'utf8'\)/);
  assert.doesNotMatch(meta,/return `cff-version:/);
  assert.match(alias,/export \{ default \} from '\.\/meta-r9\.js'/);
  assert.equal(config.functions?.['api/meta-r9.js']?.includeFiles,'CITATION.cff');
  const rewrites=new Map(config.rewrites.map(x=>[x.source,x.destination]));
  assert.equal(rewrites.get('/api/meta'),'/api/meta-r9');
  assert.equal(rewrites.get('/api/meta.js'),'/api/meta-r9');
  assert.match(cff,/^cff-version:\s*1\.2\.0/m);
  assert.match(cff,/prepublication review resource/i);
  assert.doesNotMatch(cff,/^date-released:/m);
  assert.doesNotMatch(cff,/\bdoi:\s*\S+/im);
  assert.match(cff,/^authors:\s*\n\s*-\s+name:\s*"CuHalide Atlas Project"/m);
  assert.doesNotMatch(cff,/^authors:\s*\n\s*-\s+family-names:/m);
});
