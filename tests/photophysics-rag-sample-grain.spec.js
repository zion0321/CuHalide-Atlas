...
async function activePhotoVersion(request){
  const response=await request.get(`${BASE}/api/public-data?action=photophysics-health`);
  expect(response.status()).toBe(200);
  const health=await response.json();
  const state=assertPhotophysicsHealth(health);
  expect(response.headers()['x-cuhalide-photophysics-contract']).toBe(state.version);
...
test('Research Assistant health is aligned to the live corrected photophysics baseline',async({request},testInfo)=>{
  test.skip(testInfo.project.name!=='desktop-chromium','Read-only RAG contract is viewport invariant.');
  const expected=await activePhotoVersion(request);
  ...
  const state=assertPhotophysicsHealth(x.photophysics);
  expect(state.version).toBe(expected);
...
test('two-pass Record 46 PLQY preserves article grain without false structure mapping',async({request},testInfo)=>{
  ...
  expect(x.photophysics_contract).toBe(expected);
