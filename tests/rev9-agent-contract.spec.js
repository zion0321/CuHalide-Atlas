...
for(const path of ['/api/agent','/api/agent.js'])test(`Research Assistant ${path} exposes rev.10 evidence and scientific contracts`,async({request})=>{
  ...
  expect(x).toMatchObject({ok:true,release:'3.0.2',version:'10.0.0',assistant_version:'10.6.0',current_curated_revision:10,photophysics_contract:'1.4.0',organic_components_contract:'1.2.0',publication_state:'prepublication-review',service:'CuHalide Research Assistant',operational_mode:'CONVERSATION_AND_EVIDENCE',write_access:false});
