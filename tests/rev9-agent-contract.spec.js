...
test('Research Assistant exposes the rev.9 evidence and scientific contracts',async({request})=>{
  ...
  expect(x).toMatchObject({
    ...
    operational_mode:'CONVERSATION_AND_EVIDENCE',
  ...
  expect(h['x-cuhalide-public-data-version']).toBe('2.17.1');
  expect(h['x-cuhalide-rag-version']).toBe('9.20.0');
  expect(h['x-cuhalide-assistant-version']).toBe('10.5.0');
  expect(h['x-cuhalide-photophysics-contract']).toBe('1.4.0');
  expect(h['x-cuhalide-organic-components-contract']).toBe('1.2.0');
