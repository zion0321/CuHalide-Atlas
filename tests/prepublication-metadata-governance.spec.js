...
test('Motif Atlas is independently fail-closed for indexing and dataset review status',async({request},testInfo)=>{
  ...
  expect(html).toContain('<meta name="cuhalide-publication-state" content="prepublication-review">');
  expect(html).toContain('Prepublication review · Curated through 19 Aug 2026 · rev.8');
  ...
  expect(ld).not.toBeNull();
  expect(ld.creativeWorkStatus).toBe('Prepublication review');
  expect(ld.version).toBe('current-r8');
  expect(ld.isPartOf?.version).toBe('current-r8');
});
...
test('standalone records expose review-access wording while article-page provenance stays separate from the published source',async({request},testInfo)=>{
  ...
  expect(header(article,'x-cuhalide-current-curated-revision')).toBe('8');
  const ah=await article.text();
  expect(ah).toContain('<meta name="cuhalide-publication-state" content="prepublication-review">');
  expect(ah).toContain('Current Curated rev.8 context · core article record inherited from immutable Frozen Release 3.0.2 baseline');
  ...
  expect(recordLd?.isPartOf?.version).toBe('current-r8');
  ...
  expect(header(structure,'x-cuhalide-current-curated-revision')).toBe('8');
...
test('manifest health data gateway citation assistant export and sitemap expose one rev.8 governance state',async({request},testInfo)=>{
  test.skip(testInfo.project.name!=='desktop-chromium','Governance metadata is viewport invariant; run once on desktop.');
  ...
  expect(header(manifestResponse,'x-cuhalide-current-curated-revision')).toBe('8');
  ...
  expect(manifest.current_curated).toMatchObject({revision:8,article_audit_records:383,canonical_verified_articles:369,structure_phase_rows:946,core_included_structure_rows:886,verified_space_group_rows:684,strict_polar_rows:87,strict_polar_articles:54,rag_documents:1329,rag_embedded:1329});
  ...
  expect(header(healthResponse,'x-cuhalide-current-curated-revision')).toBe('8');
  ...
  expect(health.current_curated?.live_revision).toBe(8);
  ...
  expect(header(dataResponse,'x-cuhalide-current-curated-revision')).toBe('8');
  const data=await dataResponse.json();
  expect(data.data_scope).toBe('frozen_release');
  expect(data.record_context).toMatchObject({serving_context:'current_curated',serving_revision:8,core_record_origin:'frozen_release',core_record_origin_release:'3.0.2',attached_photophysics_context:'current_curated',attached_photophysics_contract:'1.3.3'});
  ...
  expect(header(assistantResponse,'x-cuhalide-current-curated-revision')).toBe('8');
  ...
  expect(assistant.current_curated?.live_revision).toBe(8);
  ...
  expect(header(sitemapResponse,'x-cuhalide-current-curated-revision')).toBe('8');
  ...
  expect(header(cffResponse,'x-cuhalide-current-curated-revision')).toBe('8');
  ...
  expect(cff).toContain('Current Curated rev.8 (prepublication review)');
