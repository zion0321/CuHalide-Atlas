...
test('direct public-data and record compatibility paths stay on rev.10 contracts',async({request})=>{
  ...
  const r=await request.get(`${BASE}/api/record.js?kind=structure&id=CUH-091-S02`);expect(r.status()).toBe(200);const html=await r.text();expect(html).toContain('Current Curated rev.10');expect(html).toContain('<dt>Dimensionality</dt><dd>1D</dd>');currentHeaders(r.headers());
});
