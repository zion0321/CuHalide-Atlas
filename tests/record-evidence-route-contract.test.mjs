...
test('legacy public entry points and stale function suffixes are intercepted before filesystem dispatch',()=>{
  ...
  assert.match(middleware,/const isRecord=recordPaths\.has\(incoming\.pathname\)/);
  assert.match(middleware,/const isAssistantCompat=assistantCompatPaths\.has\(incoming\.pathname\)/);
