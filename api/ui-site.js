...
function replaceAll(body, from, to) {
  return typeof body === 'string' ? body.split(from).join(to) : body;
}
...
function enhanceHtml(input) {
  ...
  const replacements = [
    ...
    ['<strong>Current Curated rev.1</strong> is separately curated through 2026-08-12 and adds 14 cutoff-period coverage backfills plus 2 post-cutoff articles after primary-evidence QC.', '<strong>Current Curated rev.3</strong> is separately curated through 2026-08-14 and contains 19 snapshot-period coverage backfills plus 8 post-snapshot additions after primary-evidence QC.'],
    ...
    ['Frozen Release 3.0.2 plus primary-evidence-reviewed rev.1 additions', 'the latest primary-evidence-reviewed CuHalide Atlas corpus'],
  ...
  for (const [from, to] of replacements) body = replaceAll(body, from, to);

  body = replaceOnce(body,
    '<span><strong>CuHalide Atlas</strong><small>Evidence-grounded Cu(I) halide knowledge portal</small></span>',
    '<span><strong>CuHalide Atlas</strong><small>Cu(I) halide knowledge base</small></span>');
  ...
  body = replaceOnce(body,
    ...
    '<div class="actions"><a class="btn primary" href="#articles">Explore literature</a><a class="btn secondary" href="#structures">Browse structures</a><a class="btn secondary" href="#rag">Ask Smart RAG</a></div>');
  ...
  body = replaceOnce(body,
    ...
    '<div class="tags"><span>Curated through 14 Aug 2026</span><span>Primary-evidence reviewed</span><span>Missing values remain unresolved</span></div>');

  body = replaceOnce(body, '<span class="ver">Frozen Release 3.0.2</span>', '<span class="ver">Latest curated state</span>');
  body = body.replace(/<p class="release-note"><strong>Frozen Release 3\.0\.2<\/strong>[\s\S]*?<\/p>/,
    '<p class="release-note">Continuously updated after primary-evidence review. <a href="#citation">Archived scientific snapshot 3.0.2</a> is retained for reproducibility and historical denominator checks.</p>');
  ...
  body = replaceOnce(body, 'Reset to canonical core', 'Reset to curated literature');
  body = replaceOnce(body, 'Reset to Current canonical', 'Reset to curated literature');
  body = replaceOnce(body, 'Canonical release status = Core - Verified.', 'Latest reviewed corpus.');
  body = replaceOnce(body, 'Default data layer = Current canonical; Frozen Release core remains selectable.', 'Latest reviewed corpus.');
  body = replaceOnce(body,
    'The default view is Current Curated canonical evidence (Frozen Release 3.0.2 plus primary-evidence-reviewed Current Curated additions through rev.3). Frozen core and audit views remain explicitly selectable.',
  ...
  body = replaceOnce(body, 'Current Curated Structure Dataset Eligibility = Core - Included.', 'Latest curated structure register · Core-Included only.');
  body = replaceOnce(body, '<div class="polar-num"><strong>77</strong><small>Current strict-polar rows · 46 articles</small></div>', '<div class="polar-num"><strong>77</strong><small>strict-polar rows · 46 articles</small></div>');
  body = replaceOnce(body, '<strong id="pcount">77 rows</strong><small>Current Curated strict-polar subset; Frozen Release 3.0.2 remains 67 rows across 42 articles.</small>', '<strong id="pcount">77 rows</strong><small>Latest strict-polar subset. Polar does not imply ferroelectricity.</small>');
  ...
  body = body.replace(/<p class="fine">Workflow: discovery → DOI deduplication[\s\S]*?<\/div>/,
    '<p class="fine">Workflow: discovery → DOI deduplication → scope triage → primary article/SI/CIF review → evidence extraction → QC → curated corpus. Candidate abstracts, scores, reason codes and internal adjudication remain private.</p><div class="notice">Candidate metadata is a discovery signal, not curated scientific evidence.</div>');

  body = body.replace(/<details><summary>Smart RAG runtime boundary<\/summary><p class="fine">[\s\S]*?<\/p><\/details>/,
    '<details><summary>Smart RAG evidence boundary</summary><p class="fine">Smart RAG searches the latest reviewed evidence corpus. Exact counts, temporal scope and protected scientific boundaries remain deterministic; structure-grain identity and crystallography are kept separate from article-grain photophysics and unmapped motif interpretation. Archived temporal scopes are used only when a question explicitly requires historical reproducibility.</p></details>');
  body = body.replace(/<details><summary>Record 13 correction history<\/summary><p class="fine">[\s\S]*?<\/p><\/details>/,
    '<details><summary>Record 13 correction history</summary><p class="fine">Archived scientific snapshot 3.0.2 incorporates the four confirmed Record 13 structure-dimensionality corrections. Earlier release history is retained only for auditability; current browsing uses the corrected values.</p></details>');

  body = replaceOnce(body, '<p class="eyebrow">Citation & data availability</p><h1>Use and cite CuHalide Atlas</h1><p>The portal supports query, inspection and evidence-grounded scientific use. The complete internal curation corpus is not distributed as a public bulk download.</p>',
    '<p class="eyebrow">Data provenance & citation</p><h1>Cite the living atlas or reproduce a snapshot</h1><p>The public site always opens on the latest reviewed knowledge. An archived scientific snapshot is retained separately for exact historical reproduction.</p>');
  body = replaceOnce(body, '<p class="eyebrow">Recommended citation</p>', '<p class="eyebrow">Current portal</p>');
  body = replaceOnce(body, 'CuHalide Atlas. Frozen Release 3.0.2 (11 August 2026). https://cuhalide-atlas-v3.vercel.app/', 'CuHalide Atlas. Continuously curated Cu(I) halide knowledge portal. Curated through 14 August 2026. https://cuhalide-atlas-v3.vercel.app/');
  body = body.replace(/<p class="fine">Frozen Release 3\.0\.2 literature cutoff:[\s\S]*?<\/p>/,
  ...
  body = replaceOnce(body,
    'Evidence-grounded Cu(I) halide knowledge portal · Frozen Release 3.0.2 · cutoff through 2026-06-30 · Current Curated rev.3 through 2026-08-14',
  ...
  body = replaceOnce(body,
    ...
    "$('releaseDl').innerHTML=[['Curated through','14 Aug 2026'],['Canonical articles',cc.canonical_verified_articles||359],['Core structures',cc.core_included_structure_rows||887],['Verified SG mappings',cc.verified_space_group_rows||694],['Strict polar',cc.strict_polar_rows||77]].map(([a,b])=>`<div><dt>${a}</dt><dd>${b}</dd></div>`).join('');");
  ...
  body = replaceOnce(body,
    ...
    "$('kpis').innerHTML=[['Article audit',cc.article_audit_records||373,'reviewed DOI records'],['Canonical articles',cc.canonical_verified_articles||359,'in-scope verified'],['Structures / phases',cc.structure_phase_rows||949,'crystallographic determinations'],['Verified mappings',cc.verified_space_group_rows||694,'one-to-one SG mappings'],['Strict polar',cc.strict_polar_rows||77,`${cc.strict_polar_articles||46} articles`]].map(([a,b,c])=>`<article class=\"kpi\"><span>${a}</span><strong>${b}</strong><small>${c}</small></article>`).join('');");

  body = body.replace(/const cct=\$\('currentCuratedText'\);if\(cct\)cct\.textContent=`Current Curated through \$\{cc\.current_curated_through\|\|'2026-08-14'\} · live revision \$\{Number\(cc\.live_revision\|\|3\)\} · Frozen base \$\{cc\.base_release\|\|r\.version\|\|'3\.0\.2'\}\.`/,
    "const cct=$('currentCuratedText');if(cct)cct.textContent=`Primary-evidence reviewed through ${cc.current_curated_through||'2026-08-14'}. The public corpus updates after QC; archived snapshots remain available under Data provenance.`");

  body = replaceOnce(body,
    "$('acountNote').textContent=$('arel').value?`Release status = ${$('arel').value}.`:'Audit view: includes non-canonical records.';",
    "$('acountNote').textContent=$('arel').value==='Current canonical'?'Latest reviewed corpus.':$('arel').value?`Filtered review status: ${$('arel').options[$('arel').selectedIndex]?.text||$('arel').value}.`:'Audit view: includes boundary and excluded records.';");
  ...
  body = replaceAll(body, '<dt>Release</dt><dd>${esc(x.release_status)}</dd>', '<dt>Curation status</dt><dd>${esc([\'Core - Verified\',\'Current Curated - Verified\'].includes(x.release_status)?\'Curated\':x.release_status)}</dd>');
  body = replaceAll(body, 'Literature Watch candidate · not frozen evidence', 'Literature Watch candidate · not yet curated evidence');
  body = replaceAll(body, "'Audit view: all 949 current structure/phase rows.'", "'Audit view: all 949 structure/phase rows.'");

  body = body.replace('</head>', `${ICON_LINK}\n${STYLE_LINK}\n${LIVING_STYLE_LINK}\n</head>`)
    .replace('</body>', `${SCRIPT_LINK}\n${SCIENCE_MARKER}\n${UI_MARKER}\n</body>`);

  if (!body.includes('Latest curated state')) throw new Error('Living knowledge status card missing');
  if (!body.includes('Curated through 14 Aug 2026')) throw new Error('Public curated-through label missing');
  if (!body.includes('Archived scientific snapshot 3.0.2')) throw new Error('Archived snapshot provenance missing');
  if (body.includes('Frozen Release core · n=332')) throw new Error('Frozen snapshot must not remain a routine browsing mode');
  if (!body.includes('Curated literature · n=359')) throw new Error('Latest curated literature default missing');
  if (!body.includes('CUHALIDE_UI_V48_4_LIVING_KNOWLEDGE')) throw new Error('Living knowledge UI marker missing');
