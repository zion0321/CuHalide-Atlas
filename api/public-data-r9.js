...
function patchBody(body){if(!body)return body;try{const root=patchValue(JSON.parse(String(body)));if(root&&root.overview&&root.release)root.literature={articles:410,fulltext_articles:403,source_documents:727,text_blocks:6275,denominator:'DOI-deduplicated literature corpus'};return JSON.stringify(root)}catch{return String(body).replaceAll('Current Curated rev.9','Current Curated rev.10').replaceAll('Current Curated rev.8','Current Curated rev.10').replaceAll('current-curated-r9','current-curated-r10').replaceAll('current-curated-r8','current-curated-r10').replaceAll('current-r9','current-r10').replaceAll('current-r8','current-r10').replaceAll('2026-08-19','2026-09-14')}}
