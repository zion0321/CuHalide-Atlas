...
function patch(body){
 ...
 if(!x.includes('/ui-quality-v52.css'))x=x.replace('</head>','<link rel="stylesheet" href="/ui-quality-v52.css?v=52.3"></head>');
 if(!x.includes('/ui-quality-v52.js'))x=x.replace('</body>','<script src="/ui-quality-v52.js?v=52.3" defer></script></body>');
 ...
 x=all(x,"[['Article audit',r.article_audit_records,'all audited DOI records'],['Canonical articles',r.canonical_verified_articles,'scientific core']","[['Articles',410,'DOI-deduplicated literature corpus'],['Searchable source text',387,'current-catalog articles with searchable prose']");
...
export default async function handler(req,res){
 res.setHeader('X-CuHalide-Current-Curated-Revision',REV);res.setHeader('X-CuHalide-Site-Version',SITE);res.setHeader('X-CuHalide-UI-Version',UI);res.setHeader('X-CuHalide-Publication-State',STATE);res.setHeader('X-CuHalide-Knowledge-Contract','1.3.0');res.setHeader('Cache-Control','no-store, no-transform, max-age=0, must-revalidate');res.setHeader('Last-Modified',LAST_MODIFIED);
 const bridge={setHeader:(k,v)=>{const n=String(k).toLowerCase();if(n==='x-cuhalide-current-curated-revision')v=REV;if(n==='x-cuhalide-site-version')v=SITE;if(n==='x-cuhalide-ui-version')v=UI;if(n==='x-cuhalide-publication-state')v=STATE;if(n==='cache-control')v='no-store, no-transform, max-age=0, must-revalidate';if(n==='last-modified')v=LAST_MODIFIED;return res.setHeader(k,v)},getHeader:k=>res.getHeader?.(k),removeHeader:k=>res.removeHeader?.(k),end:body=>{const out=patch(body);if(typeof out==='string'&&out.includes('</html>'))syncCsp(out,res);res.removeHeader?.('Content-Length');return res.end(out)}};
