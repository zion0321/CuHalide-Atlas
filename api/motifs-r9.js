...
function patch(body){
  ...
  for(const a of ['rev.9','rev.8','rev.7','rev.6'])x=all(x,a,'rev.10');
  for(const a of ['Rev.9','Rev.8','Rev.7','Rev.6'])x=all(x,a,'Rev.10');
  ...
  if(!x.includes('/ui-quality-v52.css'))x=x.replace('</head>','<link rel="stylesheet" href="/ui-quality-v52.css?v=52.2"></head>');if(!x.includes('/ui-quality-v52.js'))x=x.replace('</body>','<script src="/ui-quality-v52.js?v=52.2" defer></script></body>');
...
function syncCsp(html,res){const c=String(res.getHeader?.('Content-Security-Policy')||'');if(!c)return;const hs=hashes(html);if(!hs.length)return;let next=c;if(/script-src\s+[^;]*;/i.test(next))next=next.replace(/script-src\s+[^;]*;/i,`script-src 'self' ${hs.join(' ')};`);if(/style-src\s+[^;]*;/i.test(next)){const style=String(html).match(/<style>([\s\S]*?)<\/style>/i)?.[1];if(style){const h=`'sha256-${crypto.createHash('sha256').update(style).digest('base64')}'`;next=next.replace(/style-src\s+[^;]*;/i,`style-src 'self' ${h};`)}}res.setHeader('Content-Security-Policy',next)}
