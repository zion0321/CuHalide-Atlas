...
const RELEASE='3.0.2',META_VERSION='50.5',PUBLIC_DATA_VERSION='2.16.0',ORGANIC_COMPONENTS_VERSION='1.1.0',CURRENT_REVISION='7',PUBLICATION_STATE='prepublication-review';
...
const headers=(type='application/json; charset=utf-8',photoVersion=null)=>({'Content-Type':type,'Cache-Control':'no-store, max-age=0','X-Robots-Tag':'noindex, nofollow, noarchive','X-Content-Type-Options':'nosniff','X-CuHalide-Release':RELEASE,'X-CuHalide-Meta-Version':META_VERSION,'X-CuHalide-Public-Data-Version':PUBLIC_DATA_VERSION,...(normalizePhotophysicsVersion(photoVersion)?{'X-CuHalide-Photophysics-Contract':photoVersion}:{}),'X-CuHalide-Organic-Components-Contract':ORGANIC_COMPONENTS_VERSION,'X-CuHalide-Current-Curated-Revision':CURRENT_REVISION,'X-CuHalide-Publication-State':PUBLICATION_STATE});
...
function validatePhoto(ph){try{return assertPhotophysicsHealth(ph)}catch{return null}}
function normalizeHealth(x,ph,org,photoState){
  const photoVersion=photoState?.version||null;
  ...
  x.photophysics_contract_version=photoVersion;
  ...
  x.site_probe_mode=`frontend v50 active; backend Current Curated rev.7 deterministic contract; Structured Photophysics ${photoVersion||'unavailable'} staged publication; Organic Components 1.1.0 structure-grain fail-closed depiction contract`;
  ...
  x.checks={...checks,
    photophysics_contract:photoState!==null,
    photophysics_staged_publication:photoState!==null&&ph?.publication_policy==='pass_a_curated_or_two_pass_verified',
    photophysics_private_evidence_guard:photoState!==null&&ph?.checks?.raw_primary_files_exposed===false&&ph?.checks?.raw_evidence_locators_exposed===false,
...
async function health(){
  ...
  const [runtime,photo,organic]=await Promise.all([
    ...
    fetchJsonWithRetry(PHOTOPHYSICS_HEALTH,'CuHalide-Atlas-Meta-Photophysics/migration-1.3.2-1.3.3'),
    fetchJsonWithRetry(ORGANIC_COMPONENTS_HEALTH,'CuHalide-Atlas-Meta-Organic-Components/1.1.0')
  ]);
  const photoState=validatePhoto(photo?.x),runtimeOk=runtime?.r?.ok&&runtime?.x?.ok===true,photoOk=photo?.r?.ok&&photoState!==null,organicOk=organic?.r?.ok&&organic?.x?.ok===true&&organic?.x?.contract_version===ORGANIC_COMPONENTS_VERSION;
  if(runtimeOk&&photoOk&&organicOk)return{status:200,body:JSON.stringify(normalizeHealth(runtime.x,photo.x,organic.x,photoState)),photoVersion:photoState.version};
  const ph=photo?.x||{ok:false,status:'FAIL',version:null,error:'Structured photophysics health is temporarily unavailable.'};
  const x=normalizeHealth(runtime?.x||{ok:false,status:'FAIL',error:'CuHalide Atlas deterministic runtime contract is temporarily unavailable.'},ph,organic?.x||{ok:false,status:'FAIL',contract_version:ORGANIC_COMPONENTS_VERSION,error:'Organic component health is temporarily unavailable.'},photoState);
  x.ok=false;x.retryable=true;
  return{status:503,body:JSON.stringify(x),photoVersion:photoState?.version||null};
}

async function manifest(){
  const photo=await fetchJsonWithRetry(PHOTOPHYSICS_HEALTH,'CuHalide-Atlas-Manifest-Photophysics/migration-1.3.2-1.3.3');
  const photoState=photo?.r?.ok?validatePhoto(photo?.x):null;
  if(!photoState)throw new Error('Structured photophysics manifest contract is unavailable or internally inconsistent');
  return{photoVersion:photoState.version,body:JSON.stringify({
    schema_version:'2.3',
    ...
    runtime:{site_version:'50',ui_version:'50.2',meta_version:META_VERSION,public_data_version:PUBLIC_DATA_VERSION,photophysics_contract_version:photoState.version,organic_components_contract_version:ORGANIC_COMPONENTS_VERSION,smart_rag_version:'9.19.0',research_assistant_version:'10.4.1',motif_atlas_version:'1.2'},
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(asset==='health'||asset==='health.json'){
      const h=await health();status=h.status;body=h.body;photoVersion=h.photoVersion;if(status===503)res.setHeader('Retry-After','30');
    }else if(asset==='manifest'||asset==='release-manifest.json'){
      const m=await manifest();body=m.body;photoVersion=m.photoVersion;
    ...
    for(const[k,v]of Object.entries(headers(type,photoVersion)))res.setHeader(k,v);
    if(req.method==='HEAD')return res.end();
    return res.end(body);
  }catch(error){
    console.error('[meta-v50.5-prepublication-governance]',error);
    res.statusCode=503;res.setHeader('Retry-After','30');
    for(const[k,v]of Object.entries(headers('application/json; charset=utf-8',photoVersion)))res.setHeader(k,v);
    return res.end(JSON.stringify({ok:false,error:'CuHalide Atlas metadata service is temporarily unavailable.',release:RELEASE,gateway_meta_version:META_VERSION,publication_state:PUBLICATION_STATE,public_data_version:PUBLIC_DATA_VERSION,photophysics_contract_version:photoVersion,organic_components_contract_version:ORGANIC_COMPONENTS_VERSION}));
  }
}