...
The active deterministic RAG dependency chain was migrated from the historical `cuhalide-atlas-public-data-v2` slug to canonical Public Data v3 on **2026-08-24**. The v2 slug is therefore an HTTP **410 Gone** retirement stub: it has no database/service-role capability, does not proxy v3, and cannot restore superseded v2 semantics.
...
An `ACTIVE` Supabase function can therefore still be a safe retirement stub; source behavior and JWT configuration are authoritative. Once the final live dependency is migrated, a compatibility slug must be retired rather than preserved indefinitely.
