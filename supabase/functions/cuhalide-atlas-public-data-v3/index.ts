...
const PHOTO_ALLOWED=new Set(['1.3.2','1.3.3']), ORGANIC_COMPONENTS_CONTRACT='1.1.0';
...
function photoVersion(value){const version=String(value??'').trim();if(!PHOTO_ALLOWED.has(version))throw Error(`unsupported photophysics contract ${version||'missing'}`);return version}
function maybePhotoVersion(value){const version=String(value??'').trim();return PHOTO_ALLOWED.has(version)?version:null}
...
function hdr(req,ct='application/json; charset=utf-8',photo=null){return{...cors(req),'content-type':ct,'cache-control':'no-store','x-content-type-options':'nosniff','x-robots-tag':'noindex, nofollow, noarchive','x-cuhalide-release':RELEASE,'x-cuhalide-public-data-version':VERSION,'x-cuhalide-public-access':'query-and-view','x-cuhalide-publication-state':PUBLICATION_STATE,'x-cuhalide-current-curated-revision':REV,...(maybePhotoVersion(photo)?{'x-cuhalide-photophysics-contract':String(photo)}:{}),'x-cuhalide-organic-components-contract':ORGANIC_COMPONENTS_CONTRACT}}
const send=(req,x,s=200,photo=null)=>new Response(req.method==='HEAD'?null:JSON.stringify(x),{status:s,headers:hdr(req,'application/json; charset=utf-8',photo)});
...
function projectMotifAtlas(atlas,limit=24){
  ...
  const categories=(Array.isArray(a.categories)?a.categories:[]).map(r=>({primary_category:r?.primary_category??null,structure_determinations:r?.structure_determinations??0,article_count:r?.article_count??0,identity_count:r?.identity_count??0,motif_count:r?.motif_count??0}));
  const motifs=(Array.isArray(a.motifs)?a.motifs:[]).map(r=>({primary_category:r?.primary_category??null,motif_formula:r?.motif_formula??null,structure_determinations:r?.structure_determinations??0,article_count:r?.article_count??0,identity_count:r?.identity_count??0}));
  const examples=(Array.isArray(a.examples)?a.examples:[]).slice(0,limit).map(r=>({structure_id:r?.structure_id??null,record_id:r?.record_id??null,label:r?.label??null,formula:r?.formula??null,space_group:r?.space_group??null,primary_category:r?.primary_category??null,motif_formula:r?.motif_formula??null,dimensionality:r?.dimensionality??null}));
  return{scope:a.scope??null,coverage:{total_taxonomy_rows:coverage.total_taxonomy_rows??0,motif_resolved_rows:coverage.motif_resolved_rows??0,motif_unresolved_rows:coverage.motif_unresolved_rows??0,primary_classified_rows:coverage.primary_classified_rows??0,unresolved_category_rows:coverage.unresolved_category_rows??0,unresolved_legacy_category_rows:coverage.unresolved_legacy_category_rows??coverage.unresolved_category_rows??0,label_candidate_structures:coverage.label_candidate_structures??0,curated_component_structures:coverage.curated_component_structures??0},categories,motifs,examples,counting_note:a.counting_note??null,component_note:'Detailed curated-component and label-derived candidate inventories are not part of the public Motif Atlas projection.',schema_version:a.schema_version??'1.2',public_projection:'motif-atlas-aggregate-v1',component_inventory_public:false};
}
...
Deno.serve(async req=>{
  ...
  try{
    ...
    let motifLimit=24;
    if(action==='motifs'){motifLimit=Math.max(1,Math.min(24,Number(u.searchParams.get('limit')||24)||24));u.searchParams.set('limit',String(motifLimit))}
    if(action==='photophysics-health'){
      const x=await rpc('cuhalide_atlas_public_photophysics_health_v2'),pv=photoVersion(x?.version);
      return send(req,{...x,release:RELEASE,public_data_version:VERSION,current_curated_revision:Number(REV),publication_state:PUBLICATION_STATE},200,pv);
    }
    if(action==='photophysics'){
      ...
      const x=await photophysics(id,structure),pv=photoVersion(x?.version);
      return send(req,{...x,release:RELEASE,public_data_version:VERSION,current_curated_revision:Number(REV),publication_state:PUBLICATION_STATE},200,pv);
    }
    ...
    if(r.ok&&x?.item&&(action==='article'||action==='structure')){
      ...
      if(rid){
        ...
        try{x.photophysics=await photophysics(rid,sid);attachedPhotoVersion=photoVersion(x.photophysics?.version)}catch(e){console.error('[photophysics-attach]',e);x.photophysics={ok:false,public_state:'temporarily_unavailable',samples:[],conflicts:[]}}
        if(action==='structure'&&sid){try{const items=await organicComponents([sid]);x.organic_components=items;x.item.organic_components=items}catch(e){console.error('[organic-components-attach]',e);x.organic_components=[];x.item.organic_components=[]}}
      }
    }
    return send(req,x,r.status,attachedPhotoVersion);
  }catch(e){
