...
const VERSION='retired-current-rag-r5-unified-1';
...
Deno.serve((req:Request)=>{if(!internal(req))return send({ok:false,error:'internal service authorization required'},401);return send({ok:false,status:'retired',version:VERSION,message:'Obsolete Current Curated rev.5 unified RAG wrapper retired; use the current rev.7 Research Assistant chain.'},410)});
