-- CuHalide Atlas release 3.0.1 projection relational/scientific health extension.
-- Apply after the bootstrap-health migration. This preserves the SHA-256, ACL,
-- semantic and dashboard contracts and adds relational/symmetry invariants.
...
declare
  ...
  v_duplicate_doi_groups integer;
  v_orphan_structures integer;
  v_canonical_not_included integer;
  v_unexpected_halogen_labels integer;
  v_unexpected_evidence_labels integer;
  v_polar_yes_bad_point_group integer;
  v_polar_no_polar_point_group integer;
  v_polar_yes_without_sg integer;
...
begin
  ...
  into v_structures, v_core, v_strict, v_strict_articles, v_errata
  ...
  select count(*)
  into v_duplicate_doi_groups
  from (
    select lower(trim(doi)) doi_key
    from public.cuhalide_atlas_public_articles_v301
  ...
  select count(*)
  into v_orphan_structures
  from public.cuhalide_atlas_public_structures_v301 s
  left join public.cuhalide_atlas_public_articles_v301 a
    on a.record_id=s.record_id
  where a.record_id is null;

  select count(*)
  into v_canonical_not_included
  from public.cuhalide_atlas_public_articles_v301
  where release_status='Core - Verified'
    and scope_status<>'Included';

  select count(*)
  into v_unexpected_halogen_labels
  from public.cuhalide_atlas_public_structures_v301
  where halogen_effective not in (
    'Cl','Br','I','Cl/Br','Cl/I','Br/I','Cl/Br/I','Unresolved'
  );

  select count(*)
  into v_unexpected_evidence_labels
  from public.cuhalide_atlas_public_articles_v301
  where nullif(trim(evidence_level),'') is not null
    and upper(left(trim(evidence_level),1)) not in ('A','B','C','D');

  select count(*)
  into v_polar_yes_bad_point_group
  from public.cuhalide_atlas_public_structures_v301
  where polar='Yes'
    and nullif(trim(point_group),'') is not null
    and regexp_replace(lower(trim(point_group)),'\s+','','g')
      not in ('1','2','m','mm2','3','3m','4','4mm','6','6mm');

  select count(*)
  into v_polar_no_polar_point_group
  from public.cuhalide_atlas_public_structures_v301
  where polar='No'
    and regexp_replace(lower(trim(point_group)),'\s+','','g')
      in ('1','2','m','mm2','3','3m','4','4mm','6','6mm');

  select count(*)
  into v_polar_yes_without_sg
  from public.cuhalide_atlas_public_structures_v301
  where polar='Yes'
    and nullif(trim(space_group),'') is null;
  ...
  v_anon_articles_select := has_table_privilege(
    'anon','public.cuhalide_atlas_public_articles_v301','SELECT'
  );
  v_auth_articles_select := has_table_privilege(
    'authenticated','public.cuhalide_atlas_public_articles_v301','SELECT'
  );
  v_anon_structures_select := has_table_privilege(
    'anon','public.cuhalide_atlas_public_structures_v301','SELECT'
  );
  v_auth_structures_select := has_table_privilege(
    'authenticated','public.cuhalide_atlas_public_structures_v301','SELECT'
  );
  v_anon_contract_select := has_table_privilege(
    'anon','public.cuhalide_atlas_public_projection_contract_v301','SELECT'
  );
  v_auth_contract_select := has_table_privilege(
    'authenticated','public.cuhalide_atlas_public_projection_contract_v301','SELECT'
  );
  ...
  v_service_articles_select := has_table_privilege(
    'service_role','public.cuhalide_atlas_public_articles_v301','SELECT'
  );
  v_service_articles_update := has_table_privilege(
    'service_role','public.cuhalide_atlas_public_articles_v301','UPDATE'
  );
  v_service_structures_select := has_table_privilege(
    'service_role','public.cuhalide_atlas_public_structures_v301','SELECT'
  );
  v_service_structures_update := has_table_privilege(
    'service_role','public.cuhalide_atlas_public_structures_v301','UPDATE'
  );
  v_service_contract_select := has_table_privilege(
    'service_role','public.cuhalide_atlas_public_projection_contract_v301','SELECT'
  );
  v_service_contract_update := has_table_privilege(
    'service_role','public.cuhalide_atlas_public_projection_contract_v301','UPDATE'
  );
  ...
  v_ok :=
   ...
   and v_duplicate_doi_groups=0
   and v_orphan_structures=0
   and v_canonical_not_included=0
   and v_unexpected_halogen_labels=0
   and v_unexpected_evidence_labels=0
   and v_polar_yes_bad_point_group=0
   and v_polar_no_polar_point_group=0
   and v_polar_yes_without_sg=0
  ...
  return jsonb_build_object(
    ...
    'counts',jsonb_build_object(
      ...
      'strict_polar_articles',v_strict_articles,
    ...
    'relational_contract',jsonb_build_object(
      'duplicate_doi_groups',v_duplicate_doi_groups,
      'orphan_structures',v_orphan_structures,
      'canonical_not_included',v_canonical_not_included,
      'unexpected_halogen_labels',v_unexpected_halogen_labels,
      'unexpected_evidence_labels',v_unexpected_evidence_labels,
      'polar_yes_bad_point_group',v_polar_yes_bad_point_group,
      'polar_no_polar_point_group',v_polar_no_polar_point_group,
      'polar_yes_without_space_group',v_polar_yes_without_sg
    ),
