-- CuHalide Atlas material-grain photophysics layer v1.7.
-- Preserve reported/calculated S1 and T1 energies as first-class photophysical values.
...
alter table public.cuhalide_atlas_photophysics_measurements
  ...
  check (
    (
      property_code not in ('plqy_fraction','internal_quantum_yield_fraction','eqe_fraction','transmittance_fraction','emission_channel_fraction')
      or (
    ...
    and (
      property_code not in ('emission_peak_nm','emission_range_nm','excitation_peak_nm','excitation_range_nm',
        'absorption_peak_nm','absorption_range_nm','absorption_edge_nm','fwhm_nm','lifetime_us','xel_lifetime_us',
        'bandgap_ev','singlet_energy_ev','triplet_energy_ev','singlet_triplet_gap_ev','activation_energy_ev',
        'relative_light_output_factor','light_yield_ph_mev','detection_limit_ngy_s','detection_limit_ngyair_s',
        'spatial_resolution_lp_mm','afterglow_ms','luminance_cd_m2','device_t50_h','cct_k')
      or (
