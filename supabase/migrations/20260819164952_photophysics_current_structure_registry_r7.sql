-- CuHalide Atlas photophysics health hotfix for Current Curated rev.7.
--
-- The photophysics staging-health function previously validated structure-exact mappings
-- against the superseded rev.6 internal structure table. rev.6 and rev.7 currently have
-- the same row count, so that stale dependency could remain invisible while still being
-- scientifically unsafe. This migration introduces a stable internal structure registry
-- and retargets the health gate to it. Future curated revisions only need to retarget the
-- registry view, rather than editing every downstream health function.

create or replace view atlas_internal.cuhalide_current_structure_registry_v1 as
select structure_id, record_id
from atlas_internal.cuhalide_public_structures_current_r7;

revoke all on atlas_internal.cuhalide_current_structure_registry_v1 from public, anon, authenticated;
grant select on atlas_internal.cuhalide_current_structure_registry_v1 to service_role;

comment on view atlas_internal.cuhalide_current_structure_registry_v1 is
  'Private stable registry of structure identifiers for the active Current Curated scientific revision. Retarget this view when the canonical internal structure revision advances.';
...
stable
security definer
set search_path to 'pg_catalog', 'public', 'atlas_internal'
as $function$
with checks as (
  select
    ...
    (select count(*) from atlas_internal.cuhalide_photophysics_sample_state_v1 s left join atlas_internal.cuhalide_current_structure_registry_v1 x on x.structure_id=s.structure_id where s.mapping_status='structure_exact' and (s.structure_id is null or x.structure_id is null)) as invalid_structure_exact_mappings,
